...
`manuscript/output/` if they don't exist, imports the example raw data
(`data-raw/import-data.R`, if `data/example-data.rds` isn't there yet),
checks the Quarto CLI is on `PATH`, then runs `targets::tar_make()` to
build the pipeline and render the manuscript.
...
data-raw/            →  raw, as-received data + import/cleaning scripts
                       (tracked in git — see data-raw/data-raw-readme.md)
data/                →  cleaned data from data-raw/import-data.R, read by
                       the pipeline's import_data target (git-ignored, not
                       regenerated by tar_make() itself)
